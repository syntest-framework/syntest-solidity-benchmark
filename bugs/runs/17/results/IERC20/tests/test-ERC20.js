...
contract('ERC20', (accounts) => {
	...
	it('test for ERC20', async () => {
		const contractUYmaq84 = await ERC20.new({from: accounts[2]});
		const valueDTGinun = BigInt("1462")
		const toiuTTdiL = accounts[5]
		const fromFxmS3g5 = accounts[1]
		const nulleTDWKc = await contractUYmaq84.totalSupply.call({from: "0x0000000000000000000000000000000000000001"});
		const nullLjfeolB = await contractUYmaq84.transferFrom.call(fromFxmS3g5, toiuTTdiL, valueDTGinun, {from: accounts[4]});

		assert.equal(nulleTDWKc, 0)
		await expect(contractUYmaq84.transferFrom.call(fromFxmS3g5, toiuTTdiL, valueDTGinun, {from: accounts[4]})).to.be.rejectedWith(Error);
	});
})