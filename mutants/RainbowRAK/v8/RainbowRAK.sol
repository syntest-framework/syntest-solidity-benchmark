...
contract RainbowRAK is IRewardDistributionRecipient
{
    ...
    function claimRewards() public updateReward(msg.sender){
        ...
        uint256 claimableReward = (reward.div(1000)).mul(980);    //User can redeem 98% of there total earned Rewards
        uint256 rewardFees = (reward.div(1000)).mul(20);         // remaining 2% is treated as rewardFees

        rakeToken.safeTransfer(owner(),rewardFees);                 //fees transferred to owner
        rakeToken.safeTransfer(userAccount, claimableReward);
    ...
    updateReward(address(0))
    {
        ...
        rakeToken.safeTransferFrom(msg.sender,address(this),reward);
