...
contract BirdOracle {
   ...
   */
    ...
    function extractAddress(string memory url) public pure returns (string memory) {
...
//        require(index >= 0, "No address found.");
    ...
    function substring(string memory str, uint startIndex) public pure returns (string memory) {
    ...
    function parseAddr(string memory str) public pure returns (address){
        bytes memory strBytes = bytes(str);
        uint160 iaddr = 0;
        uint160 b1;
        uint160 b2;
