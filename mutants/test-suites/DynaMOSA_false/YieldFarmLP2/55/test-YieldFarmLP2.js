...
//		const uinthxlDkTx = await YieldFarmLP2ucY70dz.claimable.call(addressm4CrUq7, {from: accounts[4]});
	...
	it('test for YieldFarmLP2', async () => {
		const YieldFarmLP2IPpwhU0 = await YieldFarmLP2.new({from: accounts[5]});
		const uintT2KFjok = BigInt("1407")
//		const uintZouFNYl = await YieldFarmLP2IPpwhU0.getPoolSize.call(uintT2KFjok, {from: accounts[0]});

		await expect(YieldFarmLP2IPpwhU0.getPoolSize.call(uintT2KFjok, {from: accounts[0]})).to.be.rejectedWith(Error);
	});

	it('test for YieldFarmLP2', async () => {
		const YieldFarmLP2IPpwhU0 = await YieldFarmLP2.new({from: accounts[5]});
		const addressJAcoSl6 = "0x0000000000000000000000000000000000000000"
		const uintPX4d4e5 = await YieldFarmLP2IPpwhU0.userLastEpochIdHarvested.call({from: accounts[0]});
//		const uintzBI7bLw = await YieldFarmLP2IPpwhU0.getCurrentEpoch.call({from: accounts[3]});
//		const uintnzsgkv = await YieldFarmLP2IPpwhU0.claimable.call(addressJAcoSl6, {from: accounts[0]});

		assert.equal(uintPX4d4e5, BigInt("0"))
		await expect(YieldFarmLP2IPpwhU0.getCurrentEpoch.call({from: accounts[3]})).to.be.rejectedWith(Error);
	});
})