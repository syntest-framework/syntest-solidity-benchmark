...
//		const uint256qBBKoM = await BAEVPNhYVJ.setAuctionFee.call(uintVhFfcgS, {from: accounts[3]});
	...
	it('test for BAE', async () => {
		const addresszOgi2qv = accounts[1]
		const BAEUAzQOdk = await BAE.new(addresszOgi2qv, {from: accounts[4]});
		const uintj65A2bZ = BigInt("1007")
		const uintKgLXu6I = BigInt("1975")
		const uintc2IS4Ug = BigInt("2018")
		const uintSfmxKqc = BigInt("417")
		const uintHsb7Y2e = BigInt("192")
		const uintlS2MFoZ = BigInt("1953")
//		const uint256Em8tZYQ = await BAEUAzQOdk.concludeAuction.call(uintj65A2bZ, {from: accounts[1]});
//		const uint256l8FioQT = await BAEUAzQOdk.resetAuction.call(uintSfmxKqc, uintc2IS4Ug, uintKgLXu6I, {from: accounts[5]});
//		const uint256CbRt8Mf = await BAEUAzQOdk.changePrintFee.call(uintHsb7Y2e, {from: accounts[0]});
//		const uint256R91CCf8 = await BAEUAzQOdk.buy.call(uintlS2MFoZ, {from: accounts[0]});

		await expect(BAEUAzQOdk.concludeAuction.call(uintj65A2bZ, {from: accounts[1]})).to.be.rejectedWith(Error);
	});
})