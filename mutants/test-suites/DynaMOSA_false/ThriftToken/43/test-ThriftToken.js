...
//		const bools2YIlGU = await ThriftTokenOah8L3Q.decreaseApproval.call(addressz7LCa6, uintMoB4PHk, {from: accounts[2]});
	...
	it('test for ThriftToken', async () => {
		const ThriftTokenhRGeYO = await ThriftToken.new({from: "0x0000000000000000000000000000000000000001"});
		const uintopddAAv = BigInt("1617")
		const addressp0hTVHy = accounts[0]
		const addresshYdSjeW = "0x0000000000000000000000000000000000000001"
		const uintklOhPj = BigInt("581")
		const addressMaSqQvj = accounts[4]
		const addressqqusWT = accounts[5]
		const uintS6bxvgp = BigInt("1659")
		const addressgLdkACV = accounts[1]
		const boolFDKq8KN = await ThriftTokenhRGeYO.increaseApproval.call(addressp0hTVHy, uintopddAAv, {from: "0x0000000000000000000000000000000000000001"});
		const addressDeyikO6 = await ThriftTokenhRGeYO.transferOwnership.call(addresshYdSjeW, {from: accounts[2]});
		const bool6GUrSY = await ThriftTokenhRGeYO.transfer.call(addressMaSqQvj, uintklOhPj, {from: accounts[4]});
		const addressRKU3W9p = await ThriftTokenhRGeYO.transferOwnership.call(addressqqusWT, {from: accounts[2]});
		const boolD9PyV9A = await ThriftTokenhRGeYO.increaseApproval.call(addressgLdkACV, uintS6bxvgp, {from: accounts[5]});
	});
})