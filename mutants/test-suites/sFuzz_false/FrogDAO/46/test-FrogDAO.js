...
//		const uintMby3XWC = await FrogDAOFwMl0j4.safeDiv.call(uintCkoDG0x, uintRzyc6lm, {from: accounts[4]});
	...
	it('test for FrogDAO', async () => {
		const FrogDAO4dEPx8 = await FrogDAO.new({from: accounts[0]});
		...
		const uinttufubM = await FrogDAO4dEPx8.allowance.call(addressBTqUOg, address7o05ni, {from: accounts[5]});
		const uintDI0W2Wr = await FrogDAO4dEPx8.safeMul.call(uintrx9ldsM, uintnxpSe4N, {from: accounts[0]});
		const uintIoJ9Yeu = await FrogDAO4dEPx8.allowance.call(addressGXDZYX2, addressgLY5S1U, {from: accounts[3]});
		const uintMmOiPOv = await FrogDAO4dEPx8.balanceOf.call(addressdGth8YU, {from: accounts[3]});
		const uintYgdxyRb = await FrogDAO4dEPx8.safeAdd.call(uintwV4Uq1V, uintnTIKo6, {from: "0x0000000000000000000000000000000000000001"});
