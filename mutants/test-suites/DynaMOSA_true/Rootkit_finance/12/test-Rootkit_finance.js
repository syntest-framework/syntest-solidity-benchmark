...
contract('Rootkit_finance', (accounts) => {
	...
	it('test for Rootkit_finance', async () => {
		const Rootkit_financeEYnMLAY = await Rootkit_finance.new({from: "0x0000000000000000000000000000000000000001"});
		const uintiqiJUEP = BigInt("1206")
		const addressz1dJXAc = accounts[0]
		const uintdupkNzX = BigInt("555")
		const addresss9c4ce7 = accounts[4]
		const addresssyVfyC9 = accounts[2]
		const addressl4JuUys = accounts[1]
		const addressfHwJ8xh = "0x0000000000000000000000000000000000000001"
		const boolJb5f1mS = await Rootkit_financeEYnMLAY.transfer.call(addressz1dJXAc, uintiqiJUEP, {from: accounts[1]});
		const boolkJhDdsM = await Rootkit_financeEYnMLAY.transfer.call(addresss9c4ce7, uintdupkNzX, {from: accounts[3]});
		const uintOtLK47 = await Rootkit_financeEYnMLAY.allowance.call(addressl4JuUys, addresssyVfyC9, {from: accounts[2]});
		const uint256y7GGLcw = await Rootkit_financeEYnMLAY.balanceOf.call(addressfHwJ8xh, {from: accounts[2]});
	});
})