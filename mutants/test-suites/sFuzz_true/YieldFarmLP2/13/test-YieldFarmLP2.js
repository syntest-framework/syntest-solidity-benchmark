...
//		const uintMDmItff = await YieldFarmLP2kQvo56M.massHarvest.call({from: accounts[3]});
	...
	it('test for YieldFarmLP2', async () => {
		const YieldFarmLP2RpQTNq = await YieldFarmLP2.new({from: accounts[4]});
		const uintsPOg3Um = BigInt("2027")
		const addresseYggccB = accounts[3]
		const uintqfQXmJY = await YieldFarmLP2RpQTNq.userLastEpochIdHarvested.call({from: accounts[2]});
//		const uintPgHRZad = await YieldFarmLP2RpQTNq.getEpochStake.call(addresseYggccB, uintsPOg3Um, {from: accounts[0]});

		assert.equal(uintqfQXmJY, BigInt("0"))
		await expect(YieldFarmLP2RpQTNq.getEpochStake.call(addresseYggccB, uintsPOg3Um, {from: accounts[0]})).to.be.rejectedWith(Error);
	});
})