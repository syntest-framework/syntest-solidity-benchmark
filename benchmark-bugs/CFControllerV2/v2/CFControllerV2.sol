...
The compiled contract might be susceptible to ABIDecodeTwoDimensionalArrayMemory (very low-severity), KeccakCaching
(medium-severity), EmptyByteArrayCopy (medium-severity), DynamicArrayCleanup (medium-severity),
ImplicitConstructorCallvalueCheck (very low-severity), TupleAssignmentMultiStackSlotComponents (very low-severity),
MemoryArrayCreationOverflow (low-severity), privateCanBeOverridden (low-severity),
YulOptimizerRedundantAssignmentBreakContinue0.5 (low-severity),
ABIEncoderV2CalldataStructsWithStaticallySizedAndDynamicallyEncodedMembers (low-severity) Solidity Compiler Bugs.
*/
...
import "./core/ConvexInterface.sol";
import "./core/ICurvePool.sol";
import "./core/YieldHandlerInterface.sol";
