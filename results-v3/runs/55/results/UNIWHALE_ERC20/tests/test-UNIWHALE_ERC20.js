...
contract('UNIWHALE_ERC20', (accounts) => {
	...
	it('test for UNIWHALE_ERC20', async () => {
		const UNIWHALE_ERC20k04ImIo = await UNIWHALE_ERC20.new({from: accounts[3]});
		const uintuMWud4l = BigInt("437")
		const uintXBaPUBr = BigInt("1905")
		const uintdxve2Ol = BigInt("467")
		const uintzNstIIP = BigInt("998")
		const uint256KNSOXwh = await UNIWHALE_ERC20k04ImIo.viewAirdrop.call({from: accounts[0]});
		const uint256ugFUEp7 = await UNIWHALE_ERC20k04ImIo.startAirdrop.call(uintzNstIIP, uintdxve2Ol, uintXBaPUBr, uintuMWud4l, {from: accounts[3]});
	});
})