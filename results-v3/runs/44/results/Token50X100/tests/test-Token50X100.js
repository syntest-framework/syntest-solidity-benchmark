...
contract('Token50X100', (accounts) => {
	...
	it('test for Token50X100', async () => {
		const Token50X100G5d0x5m = await Token50X100.new({from: accounts[1]});
		const addresslpLBiVc = accounts[1]
		const uintIOyWiFv = BigInt("1473")
		const addressK08CdLj = accounts[0]
		const uintzosauA4 = BigInt("49")
		const addressobgfl8X = accounts[6]
		const addresscTStET = await Token50X100G5d0x5m.transferOwnership.call(addresslpLBiVc, {from: accounts[1]});
		const boollv7MtPM = await Token50X100G5d0x5m.decreaseApproval.call(addressK08CdLj, uintIOyWiFv, {from: accounts[3]});
		const boolQuwMja0 = await Token50X100G5d0x5m.decreaseApproval.call(addressobgfl8X, uintzosauA4, {from: accounts[3]});

		assert.equal(boolQuwMja0, true)
		assert.equal(boollv7MtPM, true)
	});
})